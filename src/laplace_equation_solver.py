...
class LaplaceEquationSolver:
    ...
    def solve(self, constant_voltage: ScalarField) -> ScalarField:
        ...
        for _ in range(self.nb_iterations):
            # Pad the potential field with zeros to avoid boundary effects.
            potential = np.pad(potential, 1, 'constant', constant_values=0)
            # Compute relaxation iteration.
            potential = (potential[:-2, 1:-1] + potential[2:, 1:-1] + potential[1:-1, :-2] + potential[1:-1, 2:]) / 4
            # Replace all non-zero values of constant_voltage in potential
